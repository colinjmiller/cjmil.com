...
(function($, window, document) {
  ...
  function attachHandlers() {
    $("#answer").on('keyup', function(evt) {
      var field = $(this);
      var prev = difficulty;
      if (field.val().trim() === goal.trim()) {
        streak++;
        totalWords.push(goal.trim());
  ...
  function attachScoreHandlers() {
    $("#answer").on('keypress', function(evt) {
      if (evt.which == 13) {
        submitScore($(this).val(), streak, totalWords);
      }
    });
  }

  function submitScore(name, score, totalWords) {
    $.post("assets/scores.php", {name: name, score: score, totalWords: JSON.stringify(totalWords), csrf_token: $('#csrf-token').val()})
      .done(function(data) {
        $("ol").remove();
        $("#answer").remove();
